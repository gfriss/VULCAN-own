...
class ODESolver(object):
    ...
    def diffdf_rainout(self, y, atm, var): # var has been added by me, GF 
        """
        function of eddy diffusion including molecular diffusion and the rainout, with zero-flux boundary conditions and non-uniform grids (dzi)
        in the form of Aj*y_j + Bj+1*y_j+1 + Cj-1*y_j-1
        """
        ...
        # TEST condensation excluding non-gaseous species
        if vulcan_cfg.non_gas_sp: # total numnber density per level
            ysum = np.sum(y[:,atm.gas_indx], axis=1)
            h2o_indx = chem_funs.spec_list.index('H2O')
            h2o_l_s_indx = chem_funs.spec_list.index('H2O_l_s')
            hcn_indx = chem_funs.spec_list.index('HCN')
            y_h2o = y[:, h2o_indx]
            y_h2o_l_s = y[:, h2o_l_s_indx]
            #y_hcn = y[:, hcn_indx]
        #else: ysum = np.sum(y, axis=1)
        # TEST condensation excluding non-gaseous species
        ...
        j_cloud_bottom = 0
        for i,v in enumerate(y_h2o_l_s): # will go until the bottom of the cloud
            if v > 0: # or np.log10(v) > 0 # to account for numerically small values that should be 0
                j_cloud_bottom = i
                break
        for j in range(j_cloud_bottom):
            # do washout here, need to convert mass change to rate
            # try by dividing grid mass so getting ratios that is the same and number density ratios
            Lp =  y_h2o_l_s[j] # rain content from bottom of cloud
            f_i_l = KH*R*Tco[j] / (1 + KH*R*Tco[j])
            f = 1. # still don't know
            r_top = 1. / y[j, hcn_indx] # already scavenged, need to figure out what to put instead of 1
            # maybe need to put in the next forloop or save the scavenged factor there...
            F_i = f_i_l * (f + r_top)
            A[j][hcn_indx] += -F_i * var.dt
        ...
        for j in range(1,nz-1):
            ...
            if y_h2o[j] != 0. and y_h2o_l_s[j] > 0.: # invalid divide error + only do it for in-cloud (for now...)
                # following Giorgi et al (1986)
                #k_h2o = -1./dz_ave * ( Kzz[j]/dzi[j]*(y_h2o[j+1]+y_h2o[j])/2. + Kzz[j-1]/dzi[j-1]*(y_h2o[j]+y_h2o[j-1])/2. ) /y_h2o[j]
                #Ai[j, h2o_l_s_indx] += k_h2o
                #Ai[j, hcn_indx] += -(k_h2o/55) / (Navo*L*1e-9 + (KH*R*Tco[j])**(-1)) # actually this is k, so not sure -> double check!

                # following GEOS-Chem
                k_h2o = 1./dz_ave * ( Kzz[j]/dzi[j]*(y_h2o[j+1]+y_h2o[j])/2. + Kzz[j-1]/dzi[j-1]*(y_h2o[j]+y_h2o[j-1])/2. ) /y_h2o[j]
                Ai[j][h2o_l_s_indx] -= k_h2o
                # Pr == W*18/Navo and k = W/n so Pr/k = n*18/Navo (in my understanding of GEOS-Chem.......)
                #F = (y_h2o[j]*18 / (L*1e-6*Navo)) * (1 - np.exp(-k_h2o*var.dt))
                # swapping constant cloud water content from 1g/m=10^-6g/cm3 to using n_h2o_l_s*18/Navo
                #F = (y_h2o[j] / y_h2o_l_s[j]) * (1 - np.exp(-k_h2o*var.dt))
                # F will inevitable be greater than 1 this way...
                # changing k_h20 to k_hcn according to Giorgi et al (1986), maybe only in the exponential
                # as the first term regards water rain I think
                #k_hcn = (k_h2o/55) / (Navo*L*1e-9 + (KH*R*Tco[j])**(-1))
                #F = (y_h2o[j]*18 / (L*1e-6*Navo)) * (1 - np.exp(-k_hcn*var.dt))
                # Geoschem but with clarification from Jacob et al. (2000)
                if Tco[j] >= 268:
                    R = 1.
                else:
                    R = 0.2 # temporal value, still needs to validate but is around this value for other species
                L = y_h2o_l_s[j] / ysum[j]
                #Pr = k_h2o * y_h2o_l_s[j] / ysum[j]
                f_hcn_L = KH*L*R*Tco[j] / (1 + KH*L*R*Tco[j])
                k_hcn = R*f_hcn_L*k_h2o
    ...
    def lhs_jac_settling(self, var, atm):      
        ...
        y = var.y.copy()
        # TEST condensation excluding non-gaseous species
        if vulcan_cfg.non_gas_sp: # total numberdensity per level
            ysum = np.sum(y[:,atm.gas_indx], axis=1)
        else: ysum = np.sum(y, axis=1)
        # TEST condensation excluding non-gaseous species
        dzi = atm.dzi.copy()
    ...
    def lhs_jac_rainout(self, var, atm):      
        """
        directly constructing lhs = 1./(r*h)*sparse.identity(ni*nz) - dfdy
        jacobian matrix for dn/dt + dphi/dz = P - L (including molecular diffusion, grav setlling of particles and rainout)
        zero-flux BC:  1st derivitive of y is zero
        """
        y = var.y.copy()
        # TEST condensation excluding non-gaseous species
        ...
        KH = 10. # Henry's law constant, only for HCN for now..
        L = 1 # cloud water content, gm^-3
        R = 8.314 # gas constant, J/molK
        ...
        for j in range(1,nz-1):
            ...
            if y_h2o[j] != 0. and y_h2o_l_s[j] > 0.: # invalid divide error + only do it for in-cloud (for now...)
                # following Giorgi et al (1986)
                #k_h2o = -1./dz_ave * ( Kzz[j]/dzi[j]*(y_h2o[j+1]+y_h2o[j])/2. + Kzz[j-1]/dzi[j-1]*(y_h2o[j]+y_h2o[j-1])/2. ) /y_h2o[j]
                #dfdy[j_indx[j][h2o_l_s_indx], j_indx[j][h2o_l_s_indx]] += k_h2o
                #dfdy[j_indx[j][hcn_indx], j_indx[j][hcn_indx]] += -(k_h2o/55) / (Navo*L*1e-9 + (KH*R*Tco[j])**(-1)) # actually this is k, so not sure -> double check!
                ...
                # Pr == W*18/Navo and k = W/n so Pr/k = n*18/Navo (in my understanding of GEOS-Chem.......)
                #F = (y_h2o[j]*18 / (L*1e-6*Navo)) * (1 - np.exp(-k_h2o*var.dt))
                # swapping constant cloud water content from 1g/m3=10^-6g/cm3 to using n_h2o_l_s*18/Navo
                #F = (y_h2o[j] / y_h2o_l_s[j]) * (1 - np.exp(-k_h2o*var.dt))
                # changing k_h20 to k_hcn according to Giorgi et al (1986), maybe only in the exponential
                # as the first term regards water rain I think
                #k_hcn = (k_h2o/55) / (Navo*L*1e-9 + (KH*R*Tco[j])**(-1))
                ...
                if F >= 1.:
                    ...
                    dfdy[j_indx[j][hcn_indx], j_indx[j][hcn_indx]] += -F * var.dt
...
class Ros2(ODESolver):
    ...
    def solver(self, var, atm, para):
        ...
        if vulcan_cfg.use_moldiff == True and vulcan_cfg.use_settling == False and vulcan_cfg.use_rainout == False:
            diffdf = self.diffdf
            jac_tot = self.lhs_jac_tot
        elif vulcan_cfg.use_moldiff == True and vulcan_cfg.use_settling == True and vulcan_cfg.use_rainout == False:
            diffdf = self.diffdf_settling
            jac_tot = self.lhs_jac_settling
        elif vulcan_cfg.use_moldiff == False and vulcan_cfg.use_settling == False and vulcan_cfg.use_rainout == False:
            diffdf = self.diffdf_no_mol
            jac_tot = self.lhs_jac_no_mol
        elif vulcan_cfg.use_moldiff == True and vulcan_cfg.use_settling == True and vulcan_cfg.use_rainout == True:
            diffdf = self.diffdf_rainout
            jac_tot = self.lhs_jac_rainout
        ...
        if vulcan_cfg.use_moldiff == True and vulcan_cfg.use_settling == True and vulcan_cfg.use_rainout == True:
            df = chemdf(y,M,k).flatten() + diffdf(y, atm, var).flatten()
        else:
        ...
        yk2 = y + k1/r
        if vulcan_cfg.use_moldiff == True and vulcan_cfg.use_settling == True and vulcan_cfg.use_rainout == True:
            df = chemdf(yk2,M,k).flatten() + diffdf(yk2, atm, var).flatten()
        else:
