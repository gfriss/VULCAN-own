...
rank = CW.Get_rank()
size = CW.Get_size()
# defining network (crahcno is defualt), washout and version, only used to identify sim folders and outputs
#network = ''
network = '_ncho'
nowash = '_nowash'
...
nsim_total = np.prod(list(nsim.values()))
...
TP_folder = os.path.join(scratch, 'TP_files/star_dist{}'.format(version))
...
# star type
star_df = pf.read_stellar_data(os.path.join(scratch, 'stellar_flux/stellar_params{}.csv'.format(version)))
# get C/O ratios
C_to_O = pf.get_C_to_O_conjoint(nsim, network = network, nowash = nowash, version = version)
...
sim_not_done = pf.get_sim_not_done('star_dist_CtoO_rain{}.txt'.format(version), star_df, param_dict, C_to_O, nsim, nsim_total)
...
for i in range(rank*sim_per_rank, (rank+1)*sim_per_rank):   # paralellisation itself, it spreads the task between the CPUs
    ...
    TP_sim = '_'.join([sim, dist_sim]) + version # TP profile is the same for all C/O ratios
    sim = '_'.join([sim, dist_sim, CtoO_sim]) + version # this is the final simulation name
    sim_folder = os.path.join(scratch, sim + network + version)
    if os.path.isdir(sim_folder): # in case the folder already exists, meaning that sim is being run by another CPU
        continue
    ...
    new_mixing_file = os.path.join(scratch, 'mixing_files', 'sim_{}_CtoO{}mixing.txt'.format(pf.get_str_number(i_CtoO), version))
    ...
    with open(os.path.join(scratch, 'star_dist_CtoO_rain{}.txt'.format(version)), 'a') as f:
