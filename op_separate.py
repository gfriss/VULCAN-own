    ...
    def read_rate(self, var, atm):
        ...
        with open(vulcan_cfg.network) as f:
            ...
            for line_indx, line in enumerate(all_lines):
                ...
                # ========================================================================================
                if not line.startswith("#") and line.strip() and special_re == False and conden_re == False and photo_re == False and ion_re == False and rainout_re == False: # if not starts
                ...
                elif rainout_re == True and line.strip() and not line.startswith("#"):
                    Rindx[i] = int(line.partition('[')[0].strip())
                    Rf[i] = line.partition('[')[-1].rpartition(']')[0].strip()
                    
                    var.rainout_re_list.append(i)
                    k[i] = np.zeros(nz)
                    k[i+1] = np.zeros(nz)
...
class Integration(object):
    ...
    def rainout(self, var, atm):
        KH = 10000. # efficient Henry's law constant, only for HCN for now..
        ...
        for j in range(1,nz-1):
            ...
            #k_h2o = 0.
            for re in var.conden_re_list:
                if var.Rf[re] == 'H2O -> H2O_rain':# and 'H2O' in vulcan_cfg.condense_sp:
